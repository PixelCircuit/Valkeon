...
int main()
{
	...
	std::unique_ptr<lvk::IContext> ctx;
  {
	...
	window = lvk::initWindow("CubeMap Test", width, height);
  ...
  depthTexture = ctx->createTexture({
      .type = lvk::TextureType_2D, .format = lvk::Format_Z_F32, .dimensions = {(uint32_t)width, (uint32_t)height},
  ...
  lvk::Holder<lvk::ShaderModuleHandle> vert = loadShaderModule(ctx, "Applications/CubeMap/src/main.vert");
  lvk::Holder<lvk::ShaderModuleHandle> frag = loadShaderModule(ctx, "Applications/CubeMap/src/main.frag");
  lvk::Holder<lvk::ShaderModuleHandle> vertSkyBox = loadShaderModule(ctx, "Applications/CubeMap/src/skybox.vert");
  lvk::Holder<lvk::ShaderModuleHandle> fragSkyBox = loadShaderModule(ctx, "Applications/CubeMap/src/skybox.frag");
  ...
  lvk::Holder<lvk::RenderPipelineHandle> pipeline = ctx->createRenderPipeline({ .vertexInput = vdesc, 
      .smVert = vert,
      .smFrag = frag,
